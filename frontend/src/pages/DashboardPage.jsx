...
export default function DashboardPage() {
    ...
    const fetchStudents = async () => {
        try {
            ...
            // Stats
            const gradeMap = {};
            data.forEach(s => { gradeMap[s.grade] = (gradeMap[s.grade] || 0) + 1; });
    ...
    const filtered = students.filter(s =>
        `${s.firstName} ${s.lastName} ${s.grade} ${s.phoneNumber}`.toLowerCase().includes(search.toLowerCase())
    );

    const gradeColors = ['#6366f1', '#8b5cf6', '#ec4899', '#f59e0b', '#10b981', '#3b82f6', '#ef4444', '#14b8a6'];
            ...
            <div className="dashboard-content">
                ...
                <div className="stats-grid">
                    ...
                    <div className="stat-card secondary">
                        <div className="stat-icon">🏫</div>
                        <div className="stat-info">
                            <div className="stat-value">{Object.keys(stats.grades).length}</div>
                ...
                {Object.keys(stats.grades).length > 0 && (
                    <div className="grade-pills">
                        {Object.entries(stats.grades).map(([grade, count], i) => (
                            <span key={grade} className="grade-pill" style={{ background: gradeColors[i % gradeColors.length] }}>
                                {grade}: {count} сурагч
                            </span>
                        ))}
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="empty-state">
                        ...
                        <h3>{search ? 'Хайлтад тохирох сурагч олдсонгүй' : 'Сурагч байхгүй байна'}</h3>
                        <p>{search ? 'Өөр түлхүүр үгээр хайна уу' : 'Дээрх товчийг дарж эхний сурагчаа нэмнэ үү'}</p>
                    </div>
                ) : (
                            ...
                            <tbody>
                                {filtered.map((student, idx) => (
                                    <tr key={student.id} className="student-row">
                                        ...
                                        <td>
                                            <div className="student-name-cell">
                                                {student.imageUrl ? (
                                                    ...
                                                    <div className="student-avatar" style={{ background: gradeColors[idx % gradeColors.length] }}>
                                                        {student.firstName?.[0]}{student.lastName?.[0]}
                                                    </div>
                                                )}
                                                <div>
                                                    <div
                                                        className="student-fullname student-name-link"
                                        ...
                                        <td><span className="grade-tag">{student.grade}</span></td>
